class InvoiceAllocation < ActiveRecord::Base
  ...
  scope :invoice_paid_on,
    joins(:invoice).select("*, invoices.updated_at as updated, invoice_allocations.amount as allocated_amount, invoice_allocations.contribution as contribution").order("updated DESC")
  ...
  def can_reverse_transaction?
    new_amount = amount * (1-contribution)
    transaction = self.account.transactions.new(amount: -new_amount, description: "reverse payment from account #{self.name}", date: today_in_zone(account.company))
    transaction.valid?
  end

  def reverse_payment
    contribution_amount = self.amount * self.contribution
    remuneration_amount = self.amount - contribution_amount
    if self.team_account_id
      ...
      self.account.reverse_payment remuneration_amount
    else
      self.account.reverse_payment remuneration_amount

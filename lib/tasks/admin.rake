begin
  ...
  namespace :admin do
    ...
    task :check_gravatars => :environment do
      @people = Enspiral::CompanyNet::Person.all
      @people_with = Enspiral::CompanyNet::Person.all(:conditions => ["has_gravatar = ?", true])
      @people_without = Enspiral::CompanyNet::Person.all(:conditions => ["has_gravatar = ?", false])

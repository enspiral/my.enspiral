...
describe "Staff::ProjectBookings" do
  ...
  describe "GET /dashboard" do
    ...
    before(:each) do
      ...
      @project_membership = ProjectMembership.make! :project => @project, :person => @person

      @project_bookings = Array.new
      for i in (0..8) do
        pb = ProjectBooking.make! :project_membership => @project_membership, :time => rand(40), :week => Date.today + (i).weeks
    ...
    it "Shows a dashboard of the users capacity defaulted to the following weeks" do
      ...
      page.should have_content('Your Free Time')
    ...
    it 'should prompt the user if they have not yet setup a default availability' do
      @person.default_hours_available = nil
      @person.save!

      visit staff_capacity_path

      page.should have_content("You have not yet specified how many hours you work")
  ...
  describe 'GET /edit' do
    ...
    it 'should be able to save a set of project bookings' do
      visit staff_capacity_edit_path :project_id => @project.id, :person_id => @person.id, :dates => [Date.today + 2.weeks, Date.today + 3.weeks, Date.today + 4.weeks, Date.today + 5.weeks, Date.today + 6.weeks]
      
      fill_in 'project_bookings[][time]', :with => '10'

      click_button 'Save'

      ProjectBooking.find_by_project_membership_id_and_week(@project_membership.id, (Date.today + 2.weeks).beginning_of_week).time.should eq(10)
    end
